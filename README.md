...
## 一键启动 / 停止（本机最省事）
本目录已附带两个脚本，**双击即可**，不用敲命令：
- **`start-workbench.bat`** —— 启动工作台，并自动打开浏览器到 `http://127.0.0.1:8848/`。
  （若 8848 端口已在运行，则只打开浏览器、不重复启动）
- **`stop-workbench.bat`** —— 停止工作台服务。

使用方式：
1. 双击 `start-workbench.bat` → 弹出一个服务窗口（显示运行日志）并自动打开网页，即可使用。
2. 用完想关，关闭那个服务窗口，或双击 `stop-workbench.bat`。
3. 想放到桌面更顺手：右键 `start-workbench.bat` →「发送到」→「桌面快捷方式」。

> 注：脚本里写死了用本机的托管 Python（`C:\Users\ADMIN\.workbuddy\...\python.exe`）。若换到别的电脑，把脚本里 `set PY=` 那行改成那台电脑的 python 路径（或确保 `python` 在 PATH 中）即可。
